...
Negative wording inside the main prompt is weak. Weighting negative phrases can even trigger the unwanted feature. Put negative constraints in the negative prompt area instead.
...
Replacement rule: use weighted negative terms in the negative prompt area when the interface supports it, for example `(手部畸形:1.3)`, `(五官漂移:1.25)`, `(伞面穿模:1.2)`.
...
5. **负面提示词也可以加权**  
   When the platform supports a negative prompt field, frequent failure points can be weighted there: `(手部畸形:1.3)`, `(五官漂移:1.25)`, `(伞面穿模:1.2)`.
